...
router.post("/bookings", async (req, res): Promise<void> => {
  ...
  const scheduledDate = new Date(body.data.scheduledAt);
  ...
  // Validate estimatedCost is non-negative
  if (body.data.estimatedCost < 0) {
    res.status(400).json({ error: "Estimated cost cannot be negative" });
    return;
  }

  // Validate technician exists and is available for booking
  const [tech] = await db.select({
    id: techniciansTable.id,
    currentStatus: techniciansTable.currentStatus,
    isAvailable: techniciansTable.isAvailable,
    vacationMode: techniciansTable.vacationMode,
    maxDailyBookings: techniciansTable.maxDailyBookings,
  }).from(techniciansTable)
  ...
  if (tech.vacationMode) {
    res.status(409).json({ error: "Technician is currently on vacation" });
    return;
  }

  if (!tech.isAvailable) {
    res.status(409).json({ error: "Technician is not available for booking" });
    return;
  }

  const BOOKABLE_STATUSES = new Set(["online", "emergency_only"]);
  if (!BOOKABLE_STATUSES.has(tech.currentStatus)) {
    res.status(409).json({ error: `Technician is currently ${tech.currentStatus} and cannot accept new bookings` });
    return;
  }

  // Enforce maxDailyBookings
  if (tech.maxDailyBookings != null && tech.maxDailyBookings > 0) {
    const todayStart = new Date();
    todayStart.setHours(0, 0, 0, 0);
    const todayBookings = await db.select({ id: bookingsTable.id })
      .from(bookingsTable)
      .where(and(
        eq(bookingsTable.technicianId, tech.id),
        sql`${bookingsTable.createdAt} >= ${todayStart}` as any,
        sql`${bookingsTable.status} != 'cancelled'` as any
      ));
    if (todayBookings.length >= tech.maxDailyBookings) {
      res.status(409).json({ error: "Technician has reached their daily booking limit" });
      return;
    }
  }
  ...
  const [category] = await db.select({ id: serviceCategoriesTable.id })
    ...
    .where(eq(serviceCategoriesTable.id, body.data.categoryId));
  if (!category) {
    res.status(400).json({ error: "Invalid service category" });
    return;
  }
...
router.patch("/bookings/:id/status", async (req, res): Promise<void> => {
  ...
  // Require finalCost when completing a job
  if (nextStatus === "completed" && body.data.finalCost == null && existing.finalCost == null && existing.estimatedCost == null) {
    res.status(400).json({ error: "finalCost is required when completing a job" });
    return;
  }
