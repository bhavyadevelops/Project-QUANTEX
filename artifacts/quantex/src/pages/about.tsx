...
export default function About() {
  ...
  const VALUES = [
    { icon: <Zap className="w-6 h-6" />,    title: t("about_val_speed_t"), desc: t("about_val_speed_d") },
    { icon: <Shield className="w-6 h-6" />, title: t("about_val_trust_t"), desc: t("about_val_trust_d") },
    { icon: <Cpu className="w-6 h-6" />,    title: t("about_val_prec_t"),  desc: t("about_val_prec_d")  },
    { icon: <Users className="w-6 h-6" />,  title: t("about_val_comm_t"),  desc: t("about_val_comm_d")  },
  ];
    ...
    <div className="min-h-screen bg-background">
      <section className="py-20 border-b border-border bg-card relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <p className="text-primary font-mono text-sm uppercase tracking-widest mb-4">{t("about_mission_label")}</p>
          <h1 className="text-4xl md:text-6xl font-bold uppercase tracking-tighter mb-6 max-w-3xl">
            {t("about_title")} <span className="text-primary">{t("about_title_hi")}</span>
          </h1>
          <p className="text-muted-foreground font-mono max-w-2xl text-lg leading-relaxed">{t("about_desc")}</p>
        </div>
      </section>
      ...
      <section className="py-16">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-16">
            {[
              { v: "500+", l: t("about_stat1") }, { v: "50K+", l: t("about_stat2") },
              { v: "4.8★", l: t("about_stat3") }, { v: "<30m", l: t("about_stat4") },
            ].map((s) => (
          ...
          <h2 className="text-2xl font-bold uppercase mb-8">{t("about_values")}</h2>
          ...
          <h2 className="text-2xl font-bold uppercase mb-8">{t("about_team")}</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-16">
            {TEAM.map((tm) => (
              <div key={tm.name} className="border border-border bg-card p-6 rounded-lg text-center hover:border-primary/40 transition-colors">
                <div className="w-16 h-16 rounded-full bg-primary/20 border border-primary/40 flex items-center justify-center mx-auto mb-4">
                  <span className="text-2xl font-bold text-primary font-mono">{tm.name.charAt(0)}</span>
                </div>
                <p className="font-bold uppercase text-sm">{tm.name}</p>
                <p className="text-xs text-muted-foreground font-mono mt-1">{tm.role}</p>
                <div className="flex items-center justify-center gap-3 mt-3">
                  <span className="text-xs font-mono text-primary flex items-center gap-1">
                    <Star className="w-3 h-3 fill-primary" /> {tm.rating}
                  </span>
                  <span className="text-xs font-mono text-muted-foreground">{tm.jobs} {t("about_jobs")}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="border border-primary/30 bg-primary/5 p-10 rounded-lg text-center">
            <h2 className="text-2xl font-bold uppercase mb-4">{t("about_cta_title")}</h2>
            <p className="text-muted-foreground font-mono mb-6">{t("about_cta_desc")}</p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Button asChild size="lg" className="uppercase font-bold px-8">
                <Link href="/book">{t("about_book_btn")} <ArrowRight className="w-4 h-4 ml-2" /></Link>
              </Button>
