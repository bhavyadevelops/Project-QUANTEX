    ...
    <div className="min-h-screen bg-background">
      <section className="py-20 border-b border-border bg-card">
        <div className="container mx-auto px-4 text-center">
          <p className="text-primary font-mono text-sm uppercase tracking-widest mb-4">{t("services_catalog")}</p>
          <h1 className="text-4xl md:text-6xl font-bold uppercase tracking-tighter mb-4">
            {t("services_title1")} <span className="text-primary">{t("services_title2")}</span>
          </h1>
          <p className="text-muted-foreground font-mono max-w-2xl mx-auto">{t("services_sub")}</p>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4">
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {categories?.map((cat) => (
                <div key={cat.id} className="group border border-border bg-card p-8 rounded-lg hover:border-primary/60 transition-all hover:shadow-lg hover:shadow-primary/5 relative overflow-hidden">
                  ...
                  <div className="relative z-10">
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-mono text-primary border border-primary/30 px-2 py-1 rounded">{t("services_on_demand")}</span>
                      <Button asChild variant="ghost" size="sm" className="text-primary hover:text-primary">
                        <Link href="/book">{t("nav_book")} <ArrowRight className="w-3 h-3 ml-1" /></Link>
                      </Button>
                    </div>
                  </div>
                </div>
      ...
      <section className="py-16 border-t border-border bg-card">
        <div className="container mx-auto px-4">
          <h2 className="text-2xl font-bold uppercase mb-8 text-center">{t("services_avail_techs")}</h2>
            ...
            {technicians?.slice(0, 4).map((tech) => (
              <div key={tech.id} className="border border-border bg-background p-5 rounded-lg text-center hover:border-primary/40 transition-colors">
                ...
                <div className={`mt-3 text-xs font-mono px-2 py-1 rounded inline-block ${tech.isAvailable ? "bg-primary/15 text-primary" : "bg-muted text-muted-foreground"}`}>
                  {tech.isAvailable ? `● ${t("nav_book")}` : "○ BUSY"}
                </div>
              </div>
            ))}
          </div>
          <div className="text-center mt-8">
            <Button asChild size="lg" className="uppercase font-bold px-8">
              <Link href="/book">{t("services_book_btn")}</Link>
            </Button>
          </div>
        </div>
      </section>
